...
export const login = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!isMatch) {
      // Increment login attempts (for security/lockout if necessary)
...
export const refreshToken = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Refresh Token Rotation checking
    if (user.refreshTokenVersion !== decoded.version) {
      // Refresh token reuse detected! Revoke all tokens for security
    ...
    // Increment token version to rotate it
    user.refreshTokenVersion += 1;
    await user.save();
...
export const forgotPassword = async (req: Request, res: Response, next: NextFunction) => {
  // Stub for password reset link sending via email/RabbitMQ
  res.status(200).json({
    success: true,

...
export class RabbitMQService {
  ...
  public static async connect(): Promise<void> {
    ...
    try {
      ...
      console.log('Connected to RabbitMQ successfully in Payment Service');
      ...
      console.error('Failed to connect to RabbitMQ in Payment Service:', err);
      setTimeout(() => this.connect(), 5000);
    }
  }
      ...
      await this.channel.consume(queue, (msg) => {
        if (msg) {
          try {
            ...
            console.error('Error processing consumed message in Payment Service:', err);
